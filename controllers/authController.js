...
exports.postEmailConfirmation = (req, res) => {
    ...
    Token.findOne({ token: req.params.token }, (error, token) => {
        if (error || !token) {
            return res.status(400).json({ error: 'invaild token or token may have expired' })
        }
        //if we found the vaild token then find the vaild user
...
exports.Signin = async (req, res) => {
    ...
    if (!user) {
        return res.status(400).json({ error: "sorry the email you provided not found in ur system, please try another" })
    }
...
exports.resetPassword=async(req,res)=>{
    ...
    if(!token){
        return res.status(400).json({error:'invilad token or token may have expired'})
    }

    //if token found then find the vaild user for that token
    ...
    user=await user.save()
    if(!user){
        return res.status(400).json({error:'failed to rest password'})
    }
    res.json({message:'password has been reset sucessfully, login to continue'})
}
